...
export default function OutstandingCases() {
  ...
  useEffect(() => {
    ...
    if (tokenExists) {
      ...
      axios
        .get(`https://localhost:7005/api/Emails/OutstandingAdmin`)
        .then((response) => {
          console.log(data);
  ...
  return (
    <div className="my-page-home container">
      <div className="home-header">
        <h3>Outstanding Emails</h3>
        <p></p>
        <input className="form-control" placeholder="Search" readOnly={data.length<0?false:true} />
      </div>
      <hr></hr>
      {data.length <1 && <Nodata title="No data" message = "There are currently no outstanding emails" />}
      {data.length >0 && 
